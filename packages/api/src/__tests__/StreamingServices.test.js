...
describe('StreamingServices', () => {
  ...
  describe('getAllStreamingUrls', () => {
    it('returns 6 vidsrc mirrors + videasy for a movie with legacy aliases', () => {
      ...
      expect(urls.videasy).toBe(urls.server7);
      expect(urls.server7).toContain('player.videasy.net/movie/550');
    });
    ...
    it('threads season/episode through every tv server', () => {
      ...
      expect(urls.server7).toContain('/tv/1399/1/1');
    });
  });
});
