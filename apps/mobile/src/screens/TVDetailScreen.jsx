...
function StatBadge({ icon, label, value, colors }) {
  return (
  ...
  return (
    <ScrollView style={{ flex: 1 }} scrollEnabled={false}>
      ...
      <View style={styles.skeletonBody}>
        ...
        <SkeletonLoader width={220} height={18} colors={colors} style={styles.skeletonRow} />
...
export default function TVDetailScreen({ colors }) {
  ...
  useEffect(() => {
    ...
    tmdbApi
      ...
      .catch(() => {
        if (!cancelled) setError('Failed to load TV show details.');
      })
      .finally(() => {
  ...
  const similarItems = (details.similar?.results ?? details.recommendations?.results ?? [])
    .slice(0, 20)
    .map(item => tmdbApi.transformContent({ ...item, media_type: 'tv' }));
          ...
          <View style={styles.backdropContent}>
            ...
            <View style={styles.metaRow}>
              ...
              <View style={[styles.typeBadge, { backgroundColor: colors.accent }]}>
                <Text style={styles.typeBadgeText}>TV Show</Text>
              </View>
            </View>
          </View>
        </ImageBackground>
